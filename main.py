...
def configure():
    ...
    flags.DEFINE_integer('save_interval', 10, 'save the checkpoint when epoch MOD save_interval == 0') # was 10
    flags.DEFINE_integer('first_num_filters', 16, 'number of classes')
    flags.DEFINE_float('weight_decay', 2e-4, 'weight decay rate')
    flags.DEFINE_string('modeldir', 'model_v2_test', 'model directory')
    ### END CODE HERE
