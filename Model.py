...
class Cifar(object):
    ...
    def setup(self, training):
        ...
        if training:
            ...
            ### YOUR CODE HERE
            # cross entropy
            cross_ent_loss = tf.nn.softmax_cross_entropy_with_logits(labels=self.labels,
                                                                      logits=logits)
            cross_ent_loss = tf.reduce_mean(cross_ent_loss)

            # final loss function
            self.losses = cross_ent_loss + l2_loss
            ...
            #         [v for v in tf.trainable_variables() if 'batch_normalization' in v.name]
    ...
    def train(self, x_train, y_train, max_epoch):
        ...
        for epoch in range(1, max_epoch+1):
            ...
            for i in range(num_batches):
                ...
                x_batch = x_train[i*self.conf.batch_size:(i+1)*self.conf.batch_size, :]
                x_batch = np.concatenate([parse_record(x, training=True).reshape(-1, 32, 32, 3) \
                                           for x in x_batch], axis=0)

                # make y label one-hot
                y_batch_raw = y_train[i*self.conf.batch_size:(i+1)*self.conf.batch_size]
                ...
                loss, _, _ = self.sess.run(
                            [self.losses, self.train_op, self.extra_update_ops], feed_dict=feed_dict)
    ...
    def test_or_validate(self, x, y, checkpoint_num_list):
        ...
        for checkpoint_num in checkpoint_num_list:
            ...
            for i in tqdm(range(x.shape[0])):
                ...
                # Construct the current batch.
                # Don't forget to use "parse_record" to perform data preprocessing.
                x_in = parse_record(x[i], training=False).reshape(-1,32,32,3)


                ### END CODE HERE

                # Run
                feed_dict = {self.inputs: x_in}
                pred = self.sess.run(
                            [self.preds], feed_dict=feed_dict)
                preds.append(pred[0])
                ### END CODE HERE
