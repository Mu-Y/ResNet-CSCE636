...
class ResNet(object):
    ...
    def _output_layer(self, inputs, training):
        ...
        ### YOUR CODE HERE
        outputs = tf.reduce_mean(inputs, axis=[1,2], keepdims=True)  # global avg pooling
    ...
    def _stack_layer(self, inputs, filters, block_fn, strides, training):
        ...
        for i in range(self.resnet_size):
            if i==0:
                ...
                outputs = block_fn(outputs, filters_out, training, None, 1)
    ...
    def _bottleneck_block_v2(self, inputs, filters, training, projection_shortcut, strides):
        ...
        inputs = tf.layers.conv2d(inputs=inputs,
                                          ...
                                          padding='same')
        inputs = self._batch_norm_relu(inputs, training)
        # last conv layer should have #filters = filters_out
        inputs = tf.layers.conv2d(inputs=inputs,
                                          filters=filters,
                                          kernel_size=1,
                                          strides=1,
